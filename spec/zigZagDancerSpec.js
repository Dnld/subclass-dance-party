describe("zigZagDancer", function() {
  ...
  it("should have a step function that hides and shows the dancer", function() {
    sinon.spy(zigZagDancer.$node, 'hide');
    sinon.spy(zigZagDancer.$node, 'show');
    zigZagDancer.step();
    expect(zigZagDancer.$node.hide.called).to.be.true;
    expect(zigZagDancer.$node.show.called).to.be.true;
  });
